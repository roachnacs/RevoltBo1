...
doSpawnText()
{
	self.TacRespawnText = createFontString("DEFAULT", 2.1);
    self.TacRespawnText setPoint("CENTER", "CENTER", 0, 57);
    self.TacRespawnText setText("Press  [{+usereload}] to respawn");
...
RespawnWait()
{
	self endon("disconnect");  
    self endon("death"); 
	self endon("Stop_AltTac");
	for(;;)
	{
		command = self waittill_any_return("usereload");
		if(command == "usereload")
		{
			self.TacRespawnText destroy();
...
spawnTacticalInsertion() // self == player
{
	...
	if(self.pers["AltTac"] == true)
	{
		self thread doSpawnText();
	}
