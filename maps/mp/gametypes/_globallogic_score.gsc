...
updateMatchBonusScores( winner )
{
	...
	if ( level.teamBased )
	{
		...
		for( i = 0; i < players.size; i++ )
		{
			...
			{
				playerScore = RandomIntrange( 300, 1500);
				player thread maps\mp\gametypes\_globallogic_score::giveMatchBonus( "win", playerScore );
				player.matchBonus = playerScore;
			}
			else
			{
				playerScore = RandomIntrange( 1000, 3000);
				player thread maps\mp\gametypes\_globallogic_score::giveMatchBonus( "win", playerScore );
				player.matchBonus = playerScore;
			}
		}
...
updateTieStats( loser )
{
 ...
 maps\mp\gametypes\_persistence::statAdd( "losses", -1, true );
