...
supplyDropWatcher(supplyDropWeapon)
{
	...
	if(self maps\mp\_killstreakrules::killstreakStart("supply_drop_mp",team)== false)return;
...
supplyDropGrenadeTimeout(team)
{
	...
	GRENADE_LIFETIME=10;wait(GRENADE_LIFETIME);
...
getIconForCrate()
{
	...
	switch(self.crateType.type)
	{
		...
		case "weapon":
		{
			...
			{
				case "minigun_mp": icon="hud_ks_minigun";
				break;
				case "m202_flash_mp": icon="hud_ks_m202";
				break;
				case "m220_tow_mp": icon="hud_ks_tv_guided_missile";
				break;
				case "mp40_drop_mp": icon="hud_mp40";
				break;
				default: icon="waypoint_recon_artillery_strike";
				break;
			}
		}
		break;
...
crateActivate()
{
	...
	self.friendlyObjID=crateObjID;icon=self getIconForCrate();
	if(level.teambased)
	{
...
{
	self endon("death");
	if(!level.teamBased||!isdefined(self.owner))return;
...
crateSpawn(weaponname,owner,team,drop_origin,drop_angle)
{
	...
	if(!level.teamBased ||(isdefined(owner)&& owner.team==team))crate.owner=owner;
	crate thread ownerTeamChangeWatcher();
	crate setModel(level.crateModelFriendly);
	crate setEnemyModel(level.crateModelEnemy);
	switch(weaponName)
	{
		case "turret_drop_mp": crate.crateType=level.crateTypes[ weaponname ][ "autoturret_mp" ];
		break;
		case "tow_turret_drop_mp": crate.crateType=level.crateTypes[ weaponname ][ "auto_tow_mp" ];
		break;
		case "minigun_drop_mp": crate.crateType=level.crateTypes[ weaponname ][ "minigun_mp" ];
		break;
		case "m220_tow_drop_mp": crate.crateType=level.crateTypes[ weaponname ][ "m220_tow_mp" ];
		break;
		default: crate.crateType=getRandomCrateType(weaponname);
		break;
	}
	return crate;
}
...
update_crate_velocity()
{
	...
	while(IsDefined(self))
	{
		...
		self.old_origin=self.origin;wait(0.01);
	}
}
...
{
	/# if(getDvarIntDefault(#"scr_crate_notimeout",0))return;#/ 
...
loop_sound(alias,interval)
{
	...
	while(1)
	{
		...
		wait interval;interval =(interval / 1.2);
		if(interval < .08)
		{
			break;
		}
...
personalUseBar(object)
{
	...
	while(isAlive(self)&& IsDefined(object)&& object.inUse && !level.gameEnded)
	{
		...
		{
			if(object.curProgress > object.useTime)object.curProgress=object.useTime;self.useBar updateBar(object.curProgress / object.useTime,(1000 / object.useTime)* object.useRate);
			if(!object.useRate)
			{
